...
2. Must be able to "prune" a branch by moving the tip down to a selected node along a branch. This should remove all nodes "above"
   ...
   a question is, which branch name is retained for the new tip? The answer is that the first branch in the graph that contains
   the from_node will be the branch that is retained. This is the operation of backing up to a particular node and starting
   over zooming from there. Implement in Bigraph class as prune(from_node=?) method. {done}

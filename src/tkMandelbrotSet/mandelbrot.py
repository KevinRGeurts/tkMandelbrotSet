...
class MandelbrotSet(object):
    ...
    def get_plot_data(self, invert_z = False, normalize=False):
        """
        ...
        :parameter normalize: If True, then z-values will be normalized, as boolean
        :return: as tuple:
        ...
        # Normalize the z-values, if requested
        if normalize:
            self._normalize_z(_z)

        return (_x, _y, _z)

    # TODO: Finish testing this method before using, because this may not be working as intended.
    # TODO: See if this method can be sped up, after quantifying the time it takes to run.
    def _normalize_z(self, zvals):
        """
        WARNING: This function may not work as intended, and it is slow.
        Utility function called to normalize the z-values for the Mandelbrot set.
        Normalization Methodology: Equally distribute z-values to the same overall area of the plot.
        Based on: https://en.wikipedia.org/wiki/Plotting_algorithms_for_the_Mandelbrot_set (see Histogram Coloring section of wiki page)
        :parameter zvals:
            On entry: Raw z-values, z[j][i] = the zth value at the jth value of the imaginary axis and the ith value of the real axis, as [[int]]
            On exit: Normalized z-values, z[j][i] = the zth value at the jth value of the imaginary axis and the ith value of the real axis, as [[int]]
        :return: None
        """
        # Create array to store a histogram of pnts with each raw z-value
        num_pnts_with_z=array('f') # Important that this is float type, because of division below.
        for i in range(self.max_iters+1):
            num_pnts_with_z.append(0)
        # Iterate through zvals to populate the histogram data
        for j in range(self.pts_imag):
            for i in range(self.pts_real):
                zi = zvals[j][i]
                num_pnts_with_z[zi] += 1
        # Determine the total count of the historgram data.
        # TODO: Verify this should always equal self.pts_real * self.pts_imag, and eliminate the loop if possible.
        total = 0
        for i in range(self.max_iters+1):
            total += num_pnts_with_z[i]
        # Normalize the histogram data using the total
        for i in range(self.max_iters):
            num_pnts_with_z[i] = self.max_iters * num_pnts_with_z[i] / total # Here is where we need floating point division.
        # Apply the normalization to z_raw. Note that this is a "cumulative" normalization.
        for j in range(self.pts_imag):
            for i in range(self.pts_real):
                zi = zvals[j][i]
                zvals[j][i] = 0 # At this point we are overwriting the input data
                for k in range(zi): # Here is the "cumulative" part.
                    zvals[j][i] += num_pnts_with_z[k]
        # At this point zvals is a list of lists of floats, not ints.
        # Turn it back to ints.
        for j in range(self.pts_imag):
            for i in range(self.pts_real):
                zvals[j][i] = round(zvals[j][i])
        return None
...
def plot_mandelbrot_set(x, y, z, color_map='nipy_spectral', bare=False):
    ...
    :parameter z: iterations for divergence at each point, as a list of lists of ints, in column major order, such that,
                    z[j][i] = the zth value at the jth value of the imaginary axis and the ith value of the real axis
    :parameter bare: If True, just show the data content part of the plot, suitable for exporting as a picture, as boolean
    :return: The plot figure, as matplotlib.figure.Figure object 
    """
    # Create the plot for visualizing the set
    figure = plt.figure(figsize=(10,8), dpi=100) # figsize=(width in inches, height in inches)
